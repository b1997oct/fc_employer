...
export default function Page() {
    ...
    let { data, onResponse, setData, total, resetPagination, Pagination, pagination } = useResponse(),
        ...
        Rows = Adaptor(data),
        Ths = Adaptor(ths),
        templateSearch = ServerFunction('templateSearch', { setLoading, onResponse, setError }),
        template = ServerFunction('template', {
            setLoading,
            onResponse: () => resetPagination(),
            setError
        })

    useEffect(() => {
        templateSearch(pagination)
    }, [pagination])

    function onDelete(mid) {
        if (confirm('confirm delete')) {
            template(null, null, mid)
        }
    }
    ...
    function Wrapper(d) {
        const { updatedAt, subject, uid, email, _id } = d,
            onClick = () => setOpen(d)
    ...
    return (
        <EmailLayout active='Templates'>
            <ManageTemplate open={open} onClose={onClose} id={id} />
